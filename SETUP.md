...
주간·월간 화면 상단에는 **7열 격자 테이블**이 붙습니다. Slack이 2025년 8월 추가한 `table` 블록을 씁니다.

- 칸마다 날짜와 그날 일정이 들어가고, 분류는 색 원(🔵 장기출고, 🟢 월렌트출고, 🔴 장기반납, 🟠 월렌트반납, 🟣 제휴사출고, 🟤 정비/사고대차)으로 구분합니다
- 준비완료는 ✓, 휴가는 🌴, 오늘은 ▶ 로 표시됩니다
- 주간 격자에는 고객명까지 함께 나옵니다

**제약** — 테이블 전체 문자 수가 10,000자를 넘을 수 없습니다. 그래서 칸당 표시 건수를 8건부터 시작해 예산을 넘으면 자동으로 줄이고 나머지는 `외 N건`으로 접습니다. 하루 30건 × 31일(930건)로 테스트했을 때 4,729자였습니다.

셀 안에는 버튼을 넣을 수 없고 글자 색도 지정할 수 없습니다. 그래서 격자는 보기용이고, 조작은 아래 목록에서 합니다. 날짜별 `열기` 버튼으로 그 날 일간 화면에 들어가면 항목마다 ⋮ 메뉴가 붙습니다.

`table` 블록이 거부되는 환경에서는 자동으로 테이블만 빼고 다시 그립니다. 격자가 안 보이고 목록만 나온다면 Vercel 로그에 `table 블록이 거부됨` 경고가 남아 있을 겁니다.
