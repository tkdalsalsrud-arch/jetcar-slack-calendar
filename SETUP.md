...
홈 탭 상단에 렌더링된 캘린더가 PNG로 붙습니다. Block Kit에는 격자 레이아웃이 없어 이미지로 우회한 것입니다.

| 모드 | 이미지 |
| --- | --- |
| 주간 | 날짜 × (건수 / 출고 / 반납 / 휴가) 상세 표, 칸당 최대 6건 |
| 월간 | 7×5 격자, 칸마다 출고·반납 건수 배지 |
| 일간 | 없음 (항목이 적어 목록으로 충분) |
...
이미지 URL은 슬랙이 서버에서 직접 가져가므로 인증 헤더를 붙일 수 없습니다. 대신 `SLACK_SIGNING_SECRET`으로 서명한 값을 쿼리에 넣어 추측을 막았습니다. 서명이 없거나 틀리면 401을 돌려줍니다.

**도메인 인식** — Vercel이 주입하는 `VERCEL_PROJECT_PRODUCTION_URL`을 자동으로 씁니다. 커스텀 도메인을 붙였다면 환경변수 `PUBLIC_BASE_URL`에 `https://...` 형태로 지정하세요. 둘 다 없으면 이미지 블록이 조용히 빠지고 목록만 나옵니다.

**이미지가 안 바뀔 때** — URL에 `v=타임스탬프`를 매번 붙여 캐시를 우회하고 있습니다. 그래도 옛날 이미지가 보이면 🔄 새로고침을 누르세요.

---

## 알아둘 제약

- **격자에 차량번호까지 넣는 것은 불가능합니다.** 슬랙 홈 탭 폭이 약 700px이라 7열로 나누면 칸당 100px, 한글 8자 남짓입니다. 그래서 월간은 건수 배지, 주간은 세로 표로 나눴습니다.
