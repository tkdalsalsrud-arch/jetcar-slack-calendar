...
| `ADMIN_PASSWORD` | 휴가 승인·삭제용 관리자 비밀번호 |
...
웹앱(Apps Script)에서 하던 조작을 슬랙 안에서 그대로 할 수 있습니다.

| 기능 | 위치 |
| --- | --- |
| 일정 추가 / 수정 / 삭제 | 항목 우측 ⋮ 메뉴, 하단 ＋ 버튼 |
| 준비완료 체크 / 해제 | ⋮ → 준비완료 (출고 2종만) |
| 메모 · 완료일 저장 | ⋮ → 메모 / 완료일 |
| 일자별 복사 | 하단 📋 이 날 복사 |
| 휴가 신청 / 수정 / 삭제 | 휴가 보기 → ⋮ 메뉴 |
| 휴가 승인 / 취소 / 완료 | ⋮ → 비밀번호 입력 |
| 일간 · 주간 · 월간 전환 | 상단 드롭다운 |
| 일정 / 휴가 화면 전환 | 상단 드롭다운 |
| 구분 필터 | 상단 드롭다운 |

**일간 화면에서만 항목별 ⋮ 메뉴가 붙습니다.** 주간·월간은 날짜별 요약만 보이고, `열기` 버튼으로 그 날 일간 화면에 들어가 조작합니다. 슬랙 홈 탭은 블록 100개가 상한이라 한 화면에 수백 건의 조작 버튼을 넣을 수 없기 때문입니다.

웹앱과 **같은 시트를 봅니다.** 어느 쪽에서 바꾸든 다른 쪽에서 새로고침하면 반영됩니다.

### 격자 캘린더

주간·월간 화면 상단에 **7열 격자**가 붙습니다. Slack의 `table` 블록을 씁니다. 웹앱과 같은 구조로, 칸 안에 분류 라벨(장기출고·월렌트출고…)이 나오고 그 아래 해당 차량들이 들어갑니다.

- 분류는 색 원으로 구분 — 🔵 장기출고, 🟢 월렌트출고, 🔴 장기반납, 🟠 월렌트반납, 🟣 제휴사출고, 🟤 정비/사고대차
- 준비완료 ✓, 휴가 🌴, 오늘 ▶

**표시 정보는 자동으로 조절됩니다.** 테이블 전체 문자 수가 10,000자를 넘을 수 없다는 제약 때문인데, 무작정 자르지 않고 아래 순서로만 정보를 덜어냅니다.

| 단계 | 표시 내용 | 적용 시점 |
| --- | --- | --- |
| 0 | 분류 라벨 + 차량 + 고객명 | 주간, 또는 월간에서 하루 8건 이하 |
| 1 | 분류 라벨 + 차량 | 월간에서 하루 20건 이하 |
| 2 | 차량만 | 월간에서 하루 22건 이하 |
| 3+ | 건수 제한 (`＋N` 표시) | 하루 25건 이상 |

실측치는 이렇습니다.

| 하루 건수 | 월 총계 | 문자 수 | 잘린 항목 |
| --- | --- | --- | --- |
| 15건 | 330건 | 7,601자 | 없음 |
| **18건** | **398건** | **8,996자** | **없음** |
| 20건 | 445건 | 9,125자 | 없음 |
...
현재 운영 수준(하루 15~20건)에서는 **월간 격자에 한 건도 빠지지 않고 다 들어갑니다.** 고객명까지 보시려면 주간으로 전환하세요 — 주간은 하루 30건까지 고객명 포함해 전부 표시됩니다.

셀 안에는 버튼을 넣을 수 없고 글자 색도 지정할 수 없습니다(Slack 제약). 조작은 날짜별 `열기` 버튼으로 일간 화면에 들어가 항목별 ⋮ 메뉴에서 합니다.

`table` 블록이 거부되는 환경에서는 자동으로 테이블만 빼고 다시 그립니다. 격자가 안 보이면 Vercel 로그에 `table 블록이 거부됨` 경고가 남습니다.

---

## 알아둘 제약

- **격자 셀에는 버튼을 넣을 수 없습니다.** 조작은 일간 화면의 ⋮ 메뉴에서 합니다.
- **블록 100개 상한** 때문에 조작 메뉴는 일간 화면에만 붙습니다. 하루 30건까지 테스트했을 때 39블록으로 여유가 있습니다.
- **홈 탭은 클라이언트에 캐시됩니다.** 열어두고 방치하면 오래된 화면이 남습니다. 하단 갱신 시각을 보고 🔄 새로고침을 누르세요.
- **삭제는 되돌릴 수 없습니다.** 시트 행이 실제로 지워집니다. 확인 모달을 한 단계 뒀습니다.
